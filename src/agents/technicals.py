...
def technical_analyst_agent(state: AgentState) -> AgentState:
    ...
    """
    print(f"technical_analyst_agent", state)
    if not isinstance(state["data"], dict):
        raise ValueError(f"Expected state['data'] to be a dict, got {type(state['data'])}")
    # 处理逻辑
    data = state["data"]
    print(f"technical_analyst_agent type of data {type(data)}")
